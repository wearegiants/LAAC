...
class Themewrangler {
	...
	private static function set_styles($styles = '' ) {
		...
		foreach($load as $style) {
			...
			if( isset(self::$available_stylesheets[$style]) ) {
				...
				$src = self::$available_stylesheets[$style][0] ;
				$version = !empty( self::$available_stylesheets[$style][1] ) ? self::$available_stylesheets[$style][1] : false ;
	...
	private static function set_footer($contents) {
		...
		if(!empty($contents)) {
			...
			add_action('wp_header', 'themewrangler_add_to_footer');
		}
